...
const GetFreedom = ({ match }) => {
  ...
  const loadData = async () => {
    try {
      let res = await axios.get(
        `http://localhost:5000/getinputfreedom/${match.params.id}`
      );
  ...
  const loadimg = async () => {
    try {
      let res = await axios.get(
        `http://localhost:5000/geturls/${match.params.name="freedom"}`
      );
        ...
        <div>
          <div className="shadow p-3 mb-5 bg-white rounded">
            <Card className="bg-dark shadow text-white">
              {url
                .filter((name) => name.includes("freedom"))
                .map((urlData) => (
